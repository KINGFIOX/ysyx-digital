...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, {
    word_t t = s->snpc; s->dnpc = (src1 + imm) & (~(word_t)1); R(rd) = t;
#ifdef CONFIG_FTRACE
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (int32_t)src2 == 0 ? 0xffffffffu : (uint32_t)((int32_t)src1 / (int32_t)src2) /*TODO: 除0异常*/ );
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = src2 == 0 ? 0xffffffffu : src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (int32_t)src2 == 0 ? (uint32_t)src1 : (uint32_t)((int32_t)src1 % (int32_t)src2));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(rd) = src2 == 0 ? (uint32_t)src1 : src1 % src2);

  // I (CSR)
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, R(rd) = CSR(imm); CSR(imm) = src1);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, R(rd) = CSR(imm); CSR(imm) |= src1);

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, CSR(MCAUSE) = 11; CSR(MEPC) = s->pc; s->dnpc = CSR(MTVEC));
