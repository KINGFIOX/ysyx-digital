...
CpuTestbench::CpuTestbench(int argc, char **argv, char **env,
  ...
  emu = new Emulator(top, "./", simu_trace_file, uart_output_file, ram_file,
                     data_vlog_file);
  ram = new CpuRam(top, *main_time, &uart_status, mem_trace_file);
  if (restore_bp_time != 0) {
...
void CpuTestbench::save_model(vluint64_t main_time, const char *top_filename) {
  // Stub: no save support for stubbed Vtop model
  printf("skip saving model at %ldns to %s (not supported)\n", main_time,
         top_filename);
}

void CpuTestbench::restore_model(vluint64_t *main_time,
                                 const char *top_filename) {
  // Stub: no restore support for stubbed Vtop model
  printf("skip restoring model from %s (not supported)\n", top_filename);
}
...
void CpuTestbench::display_exit_cause(vluint64_t &main_time, int emask) {
  ...
  if (emask & status_trace_err) {
    fprintf(stderr, "%s Error(Code:0x%x)\n",
            ...
            : (emask & status_trace_err_rf) ? "Data"
                                            : "Perf",
            emask);
  }
...
void CpuTestbench::simulate(vluint64_t &main_time) {
  if (!simu_quiet)
    fprintf(stderr, "Verilator Simulation Start.\n");
...
#define EVAL ((clock = !clock), main_time += 1, this->eval(main_time))
  ...
  if (!EVAL) {
    ...
    while (true) {
      // Simulate until exit
      if ((main_time <= (save_bp_time + 1) &&
           main_time >= (save_bp_time - 1)) &&
          (break_once == 0)) {
      ...
      if (top->uart_ctr_bus[UART_BUS_ENAB] && top->uart_ctr_bus[UART_BUS_RW]) {
        ...
        case 2:
          ...
          break;
        case 3:
          if (uart_div_set == false &&
              (top->uart_ctr_bus[UART_BUS_DATAI] & 0x80) == 0x80) {
            uart_div_set = true;
          } else if (uart_div_set == true &&
                     (top->uart_ctr_bus[UART_BUS_DATAI] & 0x80) == 0) {
            if (div_reinit == true) {
              uart_config = (uart_config & 0xff000000) |
                            ((div_val_1 + div_val_2 + div_val_3) * 16);
              div_val_1 = 0;
              div_val_2 = 0;
              div_val_3 = 0;
              div_reinit = false;
          ...
          uart_config =
              (uart_config & 0x00ffffff) |
              ((3 - (top->uart_ctr_bus[UART_BUS_DATAI] & 0x3)) << 28) |
              ((top->uart_ctr_bus[UART_BUS_DATAI] & 0x4) << 25) |
              ((top->uart_ctr_bus[UART_BUS_DATAI] & 0x8) << 23) |
              (p_config << 24);
          /*
          //set bit
          uart_config = (uart_config & 0x0fffffff) | ((3 - (top->datai & 0x3))
          << 28);
          //set stop
