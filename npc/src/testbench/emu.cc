...
Emulator::Emulator(Vtop *top, const char *path, const char *file_out,
                   const char *uart_path, const char *file_in,
                   const char *data_vlog)
    : CpuTool(top), trapCode(STATE_RUNNING) {
  dm = new DiffManage();
...
void Emulator::init_ram(const char *path, const char *file_in) {
  ...
  ram = (uint8_t *)mmap(NULL, EMU_RAM_SIZE, PROT_READ | PROT_WRITE,
  ...
  if (!strcmp(img + (strlen(img) - 4), ".dat")) { // file extension: .dat
    ...
    printf("%s file format is not supported. You should use file format like "
           "xxx.dat or xxx.bin.\n",
           img);
    exit(1);
  }
