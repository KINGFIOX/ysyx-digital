...
CpuRam::CpuRam(Vtop *top, vluint64_t main_time, struct UART_STA *uart_status,
               const char *mem_path)
    : CpuTool(top) {
  ...
  if (restore_bp_time != 0) {
    ...
    // breakpoint_save(main_time,
    // "/media/desi/E266FD5F66FD34BF/loongson/work/chiplab_alpha/chiplab/sims/verilator/run_prog/test.file",
    // uart_status); exit(0);
      ...
      if (buf[0] == '@') {
        ...
        if (align >= 3) {
          for (int j = 0; j < width; j += 16) {
            *(vluint64_t *)(cur[idx]->data + (ptr & (pgsz - 1))) =
                conv_hex2int64(buf + j, 16);
          }
        } else {
          vluint64_t data = conv_hex2int64(buf, width);
          // fprintf(stderr,"set %lx:%lx(%s)\n",ptr,data,buf);
          if (align == 2) {
            *(unsigned *)(cur[idx]->data + (ptr & (pgsz - 1))) = data;
          } else if (align == 1) {
            *(short *)(cur[idx]->data + (ptr & (pgsz - 1))) = data;
          } else {
            *(char *)(cur[idx]->data + (ptr & (pgsz - 1))) = data;
          }
        }
        ptr += 1 << align;
      }
...
void CpuRam::jump(vluint64_t tag, vluint64_t idx) {
  ...
  if ((k + 1 == mem[idx].size() && tag > cur[idx]->tag) ||
      k == mem[idx].size()) {
...
#endif
  vluint64_t val =
      miss ? 0 : ((vluint64_t *)cur[idx]->data)[(a & (pgsz - 1)) >> 3];
  return val;
}
...
int CpuRam::process(vluint64_t main_time) {
  ...
  if (top->ram_wen) {
    ...
    // return
    // process_write(main_time,(top->ram_waddr),top->ram_wen,top->ram_wdata);
    // return
    // process_write128(main_time,top->ram_waddr&~0xf,top->ram_wen,top->ram_wdata);
  }
  return 0;
}

int CpuRam::breakpoint_save(vluint64_t main_time, const char *brk_file_name,
...
int CpuRam::breakpoint_restore(vluint64_t main_time, const char *brk_file_name,
                               struct UART_STA *uart_status) {
  FILE *brk_file;
...
int CpuRam::special_read() { return 0; }
...
// 64
void CpuRam::process_read64_same(vluint64_t data, vluint64_t &d) { d = data; }
// 128
...
int CpuRam::process_write(vluint64_t main_time, vluint64_t a, vluint64_t m,
                          unsigned *d) {
  a = a & ~0xf;
  write16B(a, m, d);
  return 0;
...
int CpuRam::read_random_vlog() {
  ...
  while (fgets(line, 65, data_vlog) != NULL) {
    ...
    if (byte_num == 1) {
      ...
      data_temp =
          data_temp << 24 | data_temp << 16 | data_temp << 8 | data_temp;
