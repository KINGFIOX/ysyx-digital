    ...
    if (m_rx_busy >= (1 << (m_nbits + m_nparity + m_nstop - 1))) {
      ...
      if (m_conwr >= 0) {
        ...
        if ((network) && (1 != send(m_conwr, buf, 1, 0))) {
          ...
          fprintf(stderr,
                  "ERR while attempting to write out--closing output port\n");
  ...
  if ((m_tx_state == TXIDLE) && ((network) || (m_conrd >= 0))) {
    ...
    if (pb.revents & POLLIN) {
      ...
      if (1 == nr) {
        ...
        if (!network) {
          fprintf(stderr,
                  "ERR while attempting to read in--closing input port\n");
          perror("UARTSIM::read() ");
          m_conrd = -1;
        } else {
          perror("O/S Read err:");
...
int UARTSIM::nettick(const int i_tx) { return rawtick(i_tx, true); }

int UARTSIM::fdtick(const int i_tx) { return rawtick(i_tx, false); }
