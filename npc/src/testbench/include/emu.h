...
public:
  ...
  Emulator(Vtop *top, const char *path, const char *file_out,
           const char *uart_path, const char *file_in, const char *data_vlog);
  ~Emulator();

  /* do init work such as init_difftest, init_nemuproxy */
  void init_emu(vluint64_t *main_time);
  void set_need_wakeup() { need_wakeup = true; }
