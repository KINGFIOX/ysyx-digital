...
CpuTool::CpuTool(Vtop *top) { this->top = top; }

extern char *difftest_ref_so;

void CpuTool::parse_args(int argc, char **argv, char **env) {

#define PARSE_FLAG(val, label)                                                 \
  if (strcmp(argv[i], label) == 0) {                                           \
    val = i + 1 >= argc || strcmp(argv[i + 1], "0") != 0;                      \
  }

#define PARSE_STR(val, label)                                                  \
  if (i + 1 < argc && strcmp(argv[i], label) == 0) {                           \
    val = argv[i + 1];                                                         \
  }

#define PARSE_INT(val, label)                                                  \
  if (i + 1 < argc && strcmp(argv[i], label) == 0) {                           \
    sscanf(argv[i + 1], "%d", &val);                                           \
  }

#define PARSE_INT64(val, label)                                                \
  if (i + 1 < argc && strcmp(argv[i], label) == 0) {                           \
    sscanf(argv[i + 1], "%ld", &val);                                          \
  }

#define PARSE_HEX(val, label)                                                  \
  if (i + 1 < argc && strcmp(argv[i], label) == 0) {                           \
    sscanf(argv[i + 1], "%x", &val);                                           \
  }
