...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT(
      ...
      R(rd) =
          (uint32_t)(((int64_t)(int32_t)src1 * (int64_t)(int32_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
          R(rd) = (uint32_t)(((int64_t)(int32_t)src1 * (uint64_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu, R,
          R(rd) = (uint32_t)(((uint64_t)src1 * (uint64_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div, R,
          R(rd) = (int32_t)src2 == 0
                      ? 0xffffffffu
                      : (uint32_t)((int32_t)src1 /
                                   (int32_t)src2) /*TODO: 除0异常*/);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu, R,
          R(rd) = src2 == 0 ? 0xffffffffu : src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem, R,
          R(rd) = (int32_t)src2 == 0
                      ? (uint32_t)src1
                      : (uint32_t)((int32_t)src1 % (int32_t)src2));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu, R,
          R(rd) = src2 == 0 ? (uint32_t)src1 : src1 % src2);

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N,
          NPCTRAP(s->pc, R(10)));
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak, N,
          NPCTRAP(s->pc, R(10))); // R(10) is $a0
